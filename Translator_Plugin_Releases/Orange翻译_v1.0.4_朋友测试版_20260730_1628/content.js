...
const FLOATING_PANEL_DRAG_THRESHOLD = 3;
...
let hasUserPanelPosition = false;
...
function createOrUpdateSelectionPanel(selectedText) {
  ...
  textElement.className = "translator-plugin-panel-text translator-plugin-panel-drag-region";
  textElement.textContent = selectedText || "";
  textElement.addEventListener("pointerdown", startPanelDrag);
...
function positionSelectionPanel(selectionRect) {
  ...
  if (hasUserPanelPosition && currentPanelViewportPosition) {
    currentPanelViewportPosition = clampPanelViewportPosition(
      currentPanelViewportPosition.left,
      currentPanelViewportPosition.top,
      panelRect.width,
      panelRect.height
    );
    applyPanelViewportPosition(currentPanelViewportPosition.left, currentPanelViewportPosition.top);
...
function clampPanelViewportPosition(left, top, width, height) {
  const viewportWidth = document.documentElement.clientWidth;
  const viewportHeight = document.documentElement.clientHeight;
  return {
    left: clampPanelCoordinate(left, width, viewportWidth),
    top: clampPanelCoordinate(top, height, viewportHeight)
  };
}
...
function startPanelDrag(event) {
  if (!selectionPanel || isPanelDragging || event.button !== 0) {
    return;
  }
  ...
  if (typeof event.currentTarget.setPointerCapture === "function") {
    try {
      ...
      console.warn("Failed to capture panel drag pointer:", error && error.message ? error.message : error);
    }
  }
  event.currentTarget.addEventListener("pointermove", movePanelDrag);
  event.currentTarget.addEventListener("pointerup", endPanelDrag);
  event.currentTarget.addEventListener("pointercancel", endPanelDrag);
  event.currentTarget.addEventListener("lostpointercapture", endPanelDrag);
...
function movePanelDrag(event) {
  if (!isPanelDragging || !panelDragState || panelDragState.pointerId !== event.pointerId || !selectionPanel) {
    return;
  }
  ...
  if (movedDistance >= FLOATING_PANEL_DRAG_THRESHOLD) {
    panelDragState.hasMoved = true;
  }

  const panelRect = selectionPanel.getBoundingClientRect();
  currentPanelViewportPosition = clampPanelViewportPosition(
    panelDragState.startLeft + deltaX,
    panelDragState.startTop + deltaY,
    panelRect.width,
    panelRect.height
  );
  hasUserPanelPosition = true;
  applyPanelViewportPosition(currentPanelViewportPosition.left, currentPanelViewportPosition.top);
  event.preventDefault();
  event.stopPropagation();
}
...
function endPanelDrag(event) {
  const handle = event && event.currentTarget ? event.currentTarget : null;
  if (handle) {
    handle.classList.remove("is-dragging");
    handle.removeEventListener("pointermove", movePanelDrag);
    ...
    if (event && typeof handle.releasePointerCapture === "function" && handle.hasPointerCapture && handle.hasPointerCapture(event.pointerId)) {
      try {
        handle.releasePointerCapture(event.pointerId);
      } catch (error) {
        console.warn("Failed to release panel drag pointer:", error && error.message ? error.message : error);
      }
    }
  }
...
function injectSelectionPanelStyles() {
  ...
  style.textContent = `
    ...
    #${FLOATING_PANEL_ID} .translator-plugin-panel-drag-region {
      cursor: grab;
      touch-action: none;
      user-select: none;
    }

    #${FLOATING_PANEL_ID} .translator-plugin-panel-drag-region.is-dragging {
      cursor: grabbing;
    }
